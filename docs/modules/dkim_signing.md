...
# If true, allow mismatch for messages from local IPs only
allow_hdrfrom_mismatch_local = false;

# If true, allow mismatch for messages from sign_networks only
...
# If true, sign inbound messages (not from local IPs, not authenticated)
# sign_inbound = false;
...
# Domain to use for DKIM signing for inbound mail ("header"/"envelope"/"auth")
#use_domain_sign_inbound = "header";

# Custom Lua function/code to determine signing domain
#use_domain_custom = "function(task) return 'example.com' end";
...
# Redis hash for looking up selectors by domain (optional)
...
# If true, check for DKIM_REJECT symbol and skip signing if found
check_violation = false;

# If true, skip signing messages that are classified as spam
#skip_spam_sign = false;

# Use milter headers mechanism instead of internal DKIM signature cache
use_milter_headers = false;

# Restrict this module to specific settings IDs
#allowed_ids = ["id1", "id2"];

# Exclude specific settings IDs from using this module
#forbidden_ids = ["id3"];
...
You can customize the header names using the following configuration options:

~~~hcl
# local.d/dkim_signing.conf
use_http_headers = true;

# Optional: customize header names (showing defaults)
http_sign_header = "PerformDkimSign";
http_sign_on_reject_header = "SignOnAuthFailed";
http_domain_header = "DkimDomain";
http_selector_header = "DkimSelector";
http_key_header = "DkimPrivateKey";

# If true, fall back to normal signing when HTTP headers are missing
allow_headers_fallback = false;
~~~
...
# Optional: custom path in vault (default: "dkim")
# vault_path = "secret/dkim";

# Optional: Vault KV secrets engine version (1 or 2, default: 1)
# vault_kv_version = 1;

# Optional: HTTP timeout for vault requests (default: 5.0)
# http_timeout = 5.0;
